    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <flux:subheading class="text-sm uppercase tracking-tighter">
                    {{ $order->created_at->format('d M Y, H:i') }}</flux:subheading>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-zinc-50 dark:bg-white/5 p-4 rounded-2xl">
            <div class="space-y-1">
                <flux:heading size="sm" class="uppercase opacity-50 text-[10px]">Pelanggan</flux:heading>
            ...
            <div
                class="space-y-1 border-t sm:border-t-0 sm:border-l border-zinc-200 dark:border-white/10 pt-4 sm:pt-0 sm:pl-6">
                <flux:heading size="sm" class="uppercase opacity-50 text-[10px]">Dikirim Ke</flux:heading>
        ...
        <div class="space-y-3">
            <flux:heading size="sm" class="uppercase opacity-50 text-[10px]">Daftar Barang</flux:heading>
            <div class="rounded-2xl border border-zinc-200 dark:border-white/10 overflow-hidden shadow-sm">
                <table class="w-full text-sm text-left">
                    <thead class="bg-zinc-100 dark:bg-white/10 text-zinc-500">
                        <tr>
                            <th class="px-5 py-3 font-bold uppercase text-[10px]">Produk</th>
                            <th class="px-5 py-3 text-center font-bold uppercase text-[10px]">Jml</th>
                    ...
                    <tfoot class="bg-blue-50 dark:bg-blue-900/20 border-t-2 border-blue-100 dark:border-blue-900/50">
                        <tr>
                            <td colspan="2"
                                class="px-5 py-4 text-right font-bold uppercase text-xs text-blue-700 dark:text-blue-300">
                                Total Pembayaran</td>
        ...
        @if($order->delivery_notes)
            <div class="p-4 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-400 rounded-xl">
                <flux:heading size="sm" class="text-amber-800 dark:text-amber-400 font-black uppercase text-[10px] mb-1">
                    Catatan dari Pembeli:</flux:heading>
        ...
        <div class="flex gap-3 pt-4">
            <flux:modal.close class="flex-1">
                <flux:button variant="ghost" class="w-full py-3 text-base">Tutup</flux:button>
            </flux:modal.close>

            <flux:button variant="primary" icon="printer" as="a" href="{{ route('admin.orders.print', $order->id) }}"
                target="_blank" class="flex-1 py-3 text-base">Cetak Invoice</flux:button>
        </div>
    </div>
</flux:modal>