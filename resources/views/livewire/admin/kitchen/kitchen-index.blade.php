    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl" wire:poll.5s>
        ...
        <div class="flex flex-wrap gap-2 mb-2">
            <flux:button wire:click="$set('filterType', 'all')" variant="{{ $filterType === 'all' ? 'primary' : 'ghost' }}" size="sm" class="rounded-full">
                All
            </flux:button>

            <flux:button wire:click="$set('filterType', 'cooked')" variant="{{ $filterType === 'cooked' ? 'primary' : 'ghost' }}" color="{{ $filterType === 'cooked' ? 'orange' : '' }}" size="sm" class="rounded-full flex gap-2">
                Cooked
            </flux:button>

            <flux:button wire:click="$set('filterType', 'frozen')" variant="{{ $filterType === 'frozen' ? 'primary' : 'ghost' }}" color="{{ $filterType === 'frozen' ? 'blue' : '' }}" size="sm" class="rounded-full flex gap-2">
                Frozen
            </flux:button>
        </div>
        ...
        <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            @forelse ($orders as $order)
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm flex flex-col justify-between">
                    ...
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <flux:badge 
                            color="{{ $order->order_status === 'processing' ? 'yellow' : 'zinc' }}"
                            size="sm"
                            class="uppercase">
                            {{ $order->order_status }}
                        </flux:badge>
